...
/* IAV Version benutzt die BAP_BCL_CanRxPreCopy Funktion und variable DLC  */

/**
 *  Dies ist eine ungetestete PreCopy-Version fuer CAN-Treiber, welche bei PreCopy einen
 *  Struct als Parameter haben
 *  @remarks Diese Variante wurde nicht getestet, da kein entsprechender CAN Treiber zur Verfuegung stand.
 */
#define BAP_START_SEC_CODE_FAST
#include <BAP_MemMap.h>
uint8_t
...
{
 /* @todo: Die Nummerierung bei untenstehender Funktion im Bereich aRxStruct->Channel muss entsprechend
    der Konfiguration sein. Untenstehende Nummerierung beginnt bei 0! Wurde also festgelegt, dass der erste
    CAN Controller die Nummer 1 hat, so muss untenstehend ein "aRxStruct->Channel +1" eingetragen werden.
    Je nachdem wie der OEM die Datenfestlegung erstellt muss als Channel evtl. auch immer die Konstante 1 eingetragen werden
    */

    BAP_CANUBS_RxData((BapCanInterfaceChannel_t) aRxStruct->Channel, aRxStruct->Handle, (ptr_t) aRxStruct->pChipData, CAN_RX_ACTUAL_DLC);
