 ...
 *
 *  In der Version des CAN Treibers fuer ST10 sind diese
 ...
 * 2004-12-10   1.2.1       ALA/SVDO    Anpassung an ST10 Vector Treiber
...
/**
 * Liefert den DLC der aktuell empfangenen Daten.
 * Leider bietet die Implementierung fuer den ST10 keine direkte Zugriffsfunktion an.
 * Aus dem Quellcode des TP-Moduls TP20v.c wurde folgendes extrahiert
 */
#define CanRxActualDLC(rxStruct)          ((*(rxStruct->pChipMsgObj+4)) & 0x000F)
...
/* ST10 Version benutzt die BAP_BCL_CanRxPreCopy Funktion und variablen DLC */

/**
 *  Dies ist eine PreCopy-Version fuer CAN-Treiber, welche bei PreCopy einen Struct als Parameter haben
 */
#define BAP_START_SEC_CODE_FAST
#include <BAP_MemMap.h>
uint8_t
BAP_BCL_CanRxPreCopy(CanRxInfoStructPtr aRxStruct)
{
    /* eventuell muss in spaeteren Versionen (BapCanInterfaceChannel_t) aRxStruct->Channel+1 durch 1 ersetzt werden.
       Dies haengt davon ab, wie der OEM die Datenfestlegung erstellt. Ist dort als InterfaceNumber immer 1 eingetragen, unabhaengig davon,
       von welchem CAN Anschluss die Daten kommen, muss (BapCanInterfaceChannel_t) aRxStruct->Channel+1 durch 1 ersetzt werden. */
