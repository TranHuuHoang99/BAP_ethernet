 ...
 * 2017-02-27	1.9.0       FST         [SSWCCB-1545] SSW-Lokatierungs-Pragmas eingefuegt
 ...
 * 2014-03-21	1.7.1       WWU         CCB664, CCB 1731: Alignment problem.
 *                                      Type BapBalRamRow_t ist jetzt struct, statt union.
 * 2013-01-30	1.7.0       WWU         MISRA-2004 fix
 * 2007-08-27   1.5         RVE         Tabellen fuer die Abbildung von OpCodes <-> Requests/Indications erweitert.
...
#define BAP_VOID_SIZE ((uint16_t)0u)

/** Konstante fuer die benoetigten Bytes pro uint8_t */
#define BAP_INT8_SIZE ((uint16_t)1u)

#ifdef BAP_USES_DATATYPE_16
/** Konstante fuer die benoetigten Bytes pro uint16_t */
#define BAP_INT16_SIZE ((uint16_t)2u)
#endif /* #ifdef BAP_USES_DATATYPE_16 */

#ifdef BAP_USES_DATATYPE_32
/** Konstante fuer die benoetigten Bytes pro uint32_t */
#define BAP_INT32_SIZE ((uint16_t)4u)
#endif /* #ifdef BAP_USES_DATATYPE_32 */

/** Konstante fuer den letzten Error Code in enum BapError_t */
#define BAP_MAX_ERROR_CODE 0x39

/** Konstante fuer die Anzahl von Lsg-Arten: FSG und ASG */
#define BAP_NUMBER_OF_LSGTYPES 2

/** Konstante fuer die erste erlaubte FctID im FSG nach Initialisierung */
#define BAP_SMALLEST_ALLOWED_FCTID_IN_FSG 8
...
/* Request-Opcode Matrix fuer Kombi, 1st Dimension: 0=ASG, 1=FSG */
#if defined(BAP_USES_METHODS)
extern const BapOpCodes_et BAP_ROM_CONST BAP_Request2OpCodeMatrix[BAP_NUMBER_OF_LSGTYPES][BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_REQUESTS];   	/* OpCode Matrix mit Property, Cache, Methode und Array */
#elif defined(BAP_USES_ARRAYS)
extern const BapOpCodes_et BAP_ROM_CONST BAP_Request2OpCodeMatrix[BAP_NUMBER_OF_LSGTYPES][BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_REQUESTS];	/* OpCode Matrix mit Property, Cache und Array */
#else
extern const BapOpCodes_et BAP_ROM_CONST BAP_Request2OpCodeMatrix[BAP_NUMBER_OF_LSGTYPES][BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_REQUESTS];	/* OpCode Matrix mit Property, Cache */
#endif /* BAP_USES_METHODS */

#else
/* Request-OpCode Matrix fuer ASG oder FSG*/
#if defined(BAP_USES_METHODS)
extern const BapOpCodes_et BAP_ROM_CONST BAP_Request2OpCodeMatrix[BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_REQUESTS];   /* OpCode Matrix mit Property, Cache, Methode und Array */
#elif defined(BAP_USES_ARRAYS)
extern const BapOpCodes_et BAP_ROM_CONST BAP_Request2OpCodeMatrix[BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_REQUESTS];	/* OpCode Matrix mit Property, Cache und Array */
#else
extern const BapOpCodes_et BAP_ROM_CONST BAP_Request2OpCodeMatrix[BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_REQUESTS];	/* OpCode Matrix mit Property, Cache */
#endif /* BAP_USES_METHODS */
 ...
 */
#if defined(BAP_ASG) && defined(BAP_FSG)
/* Index-OpCode Matrix fuer Kombi, 1st Dimension: 0=ASG, 1=FSG*/
#if defined(BAP_USES_METHODS)
extern const BapIndication_et BAP_ROM_CONST BAP_OpCode2IndicationMatrix[BAP_NUMBER_OF_LSGTYPES][BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_OPCODES_FOR_INDICATIONS];	/* OpCode Matrix mit Property, Cache, Methode und Array */
#elif defined(BAP_USES_ARRAYS)
extern const BapIndication_et BAP_ROM_CONST BAP_OpCode2IndicationMatrix[BAP_NUMBER_OF_LSGTYPES][BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_OPCODES_FOR_INDICATIONS];	/* OpCode Matrix mit Property, Cache und Array */
#else
extern const BapIndication_et BAP_ROM_CONST BAP_OpCode2IndicationMatrix[BAP_NUMBER_OF_LSGTYPES][BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_OPCODES_FOR_INDICATIONS];	/* OpCode Matrix mit Property, Cache */
#endif /* BAP_USES_METHODS */
#else
/* Index-Opcode Matrix fuer ASG oder FSG */
#if defined(BAP_USES_METHODS)
extern const BapIndication_et BAP_ROM_CONST BAP_OpCode2IndicationMatrix[BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_OPCODES_FOR_INDICATIONS];		/* OpCode Matrix mit Property, Cache, Methode und Array */
#elif defined(BAP_USES_ARRAYS)
extern const BapIndication_et BAP_ROM_CONST BAP_OpCode2IndicationMatrix[BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_OPCODES_FOR_INDICATIONS];		/* OpCode Matrix mit Property, Cache und Array */
#else
extern const BapIndication_et BAP_ROM_CONST BAP_OpCode2IndicationMatrix[BAP_NUMBER_OF_FUNCTIONCLASSES][BAP_NUMBER_OF_OPCODES_FOR_INDICATIONS];		/* OpCode Matrix mit Property, Cache */
#endif /* BAP_USES_METHODS */
#endif /* defined(BAP_ASG) && defined(BAP_FSG) */
...
/**
 *  Diese Funktion prueft die BAPConfig Botschaft. Es wird die Major und Minor Number
 *  der Protokollversion, die Steuergeraeteklasse und die Major Number der
 *  Datenfestlegung auf Gleichheit mit einen Eintrag aus der Referenzliste
 *  verglichen. Beim ASG wird seit BAP 1.6.4 keine Minor Nummer mehr geprueft.
 *
 *  Aktionen:
 *      - Bei einer Reset-Indication wird der Cache auf ungueltig gesetzt und
 *        Retries/Rx/Tx der unteren Schichten ausgeschaltet
 *      - Bei einer fehlerhaften Protokollversion beim ASG wird eine ErrorIndication mit dem
 *        Fehlerwert BapErr_IncompatibleProtocolVersion gemeldet (Weiterarbeit mit der Alten).
 *      - Bei einer fehlerhaften Protokollversion beim FSG wird der Zustand auf InvaildConfig gesetzt.
 ...
 *
 * @param fctId bezeichnet die Fct.Id welche auf Gueltigkeit getestet werden soll
 *
 * @param aru8FctList bezeichnet die aktuelle Funktionslist
 *
