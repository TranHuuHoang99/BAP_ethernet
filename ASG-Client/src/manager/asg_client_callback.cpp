...
{
    ASGManager::instance()->acknowledge(aLsgId, aFctId, aeAcknowledge);
}

void BAP_IndicationVoid(lsgId_t aLsgId, fctId_t aFctId, enum BapIndication_t aeIndication)
{
    ASGManager::instance()->indicationVoid(aLsgId, aFctId, aeIndication);
}
...
void BAP_IndicationInt8(lsgId_t aLsgId,
                        ...
                        uint8_t au8Value)
{
	ASGManager::instance()->indicationInt8(aLsgId, aFctId, aeIndication, au8Value);
}
...
void BAP_IndicationInt16(lsgId_t aLsgId,
                         ...
                         uint16_t au16Value)
{
	ASGManager::instance()->indicationInt16(aLsgId, aFctId, aeIndication, au16Value);
}
...
void BAP_IndicationInt32(lsgId_t aLsgId,
                         ...
                         uint32_t au32Value)
{
	ASGManager::instance()->indicationInt32(aLsgId, aFctId, aeIndication, au32Value);
}
...
void BAP_IndicationByteSequence(lsgId_t aLsgId,
                                ...
                                uint32_t au32Length)
{
	ASGManager::instance()->indicationByteSequence(aLsgId,
                                                   aFctId,
                                                   aeIndication,
                                                   apValue,
                                                   au32Length);
...
{
	ASGManager::instance()->indicationError(aLsgId, aFctId, aeErrorCode);
}

enum BapError_t BAP_SendDebugInfo(DBGVAR const uint8_t pDebugInfo[8])
{
	
    const BapError_t ret_code = ASGManager::instance()->sendDebugInfo(pDebugInfo);
    return ret_code;
}
